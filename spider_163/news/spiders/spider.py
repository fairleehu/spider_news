...
class news_spider(CrawlSpider):
    ...
    rules = [
        ...
        Rule(le(allow=(r'/\w+\.163\.com/16/05\d{2}/\d+/\w+\.html')),
             follow=True,
             callback='parse_item')
    ]
    _x_query = {
        'title': '''//div[contains(@class, 'post_content_main')]/h1/text()''',
        # "articleurl":'''//div[contains(@class, "ep-time-source cDGray")]/a[contains(@id, "ne_article_source")]/@href''',
        'content': '''//div[contains(@class, "post_text")]/p/text()''',
        'picurl': '''//div[contains(@class, "post_text")]/p[contains(@class, "f_center")]/img/@src''',
    }
    print "**********news_spider************"
