...
class Picture(models.Model):  # 保存新闻中的图片URL
    ...
    pictureID = models.TextField(
        max_length=100, blank=True, null=True)  # 与News表的中picture_id对应
...
class News(models.Model):  # 新闻表
    ...
    newsLable = models.TextField(
        max_length=20, blank=True, null=True)  # 新闻label
    ...
    picture_id = models.TextField(
        max_length=100, blank=True, null=True)  # 与Picture表对应的pictureID
...
class Comments(models.Model):  # 新闻评论表
    ...
    content_time = models.DateTimeField(
        auto_now=True)  # 自动保存时间auto_now =True，评论的时间
