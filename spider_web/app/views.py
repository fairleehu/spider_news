...
def showNews(request, newsID):  # 新闻二级页面
    ...
    comment = Comments.objects.filter(news_id=newsID).order_by(
        "content_time")  # 评论内容按content_time排序
...
def personal(request):
    ...
    cursor.execute('select distinct n.id,n.newsTitle from app_comments c,app_news n where c.user_id = %s and n.id = c.news_id',  [
                   request.user.id])
    sql = cursor.fetchall()
    for sql_res in sql:
        tmp = {}
...
def comment(request, userID, newsID):
    ...
    Comments.objects.create(user_id=userID, news_id=newsID,
                            username=username, content=request.POST['content'])  # 将评论内容保存到数据库
